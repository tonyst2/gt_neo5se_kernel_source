...
#endif
#if defined(CONFIG_UFSSSR)
	if (ufsssr_get_state(ufsf) == SSR_NEED_INIT)
		ufsssr_get_geo_info(ufsf, geo_buf);
#endif
	return 0;
}
...
void ufsf_upiu_check_for_ccd(struct ufshcd_lrb *lrbp)
{
	...
	INFO_MSG("sense : %02X %02X %02X %02X %02X %02X %02X\n",
...
#endif
#if defined(CONFIG_UFSSSR)
	if(ufsssr_get_state(ufsf) == SSR_PRESENT)
		ufsssr_remove(ufsf);
#endif
}
...
#endif
#if defined(CONFIG_UFSSSR)
	ufsssr_set_state(ufsf, SSR_NEED_INIT);
#endif
}
